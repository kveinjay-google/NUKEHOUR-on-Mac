		...
		sealed class BagFile : IReadOnlyPackage
		{
			...
			public Stream GetStream(string filename)
			{
				...
				if ((entry.Flags & 2) > 0)
				{
					...
					try
					{
						bytesPerSecond = checked(2u * (uint)channels * entry.SampleRate);
					}
					catch (OverflowException e)
					{
						throw new InvalidDataException($"Invalid PCM sample rate {entry.SampleRate} for '{filename}'.", e);
					}
					...
					waveHeaderMemoryStream.Write(BitConverter.GetBytes(bytesPerSecond));
