...
namespace OpenRA.Mods.RA2.Traits
{
	...
	public class BallisticMissileInfo : TraitInfo, IMoveInfo, IPositionableInfo, IFacingInfo, IRulesetLoaded
	{
		...
		[Desc("Use the deterministic launch, cruise, acquire and terminal-dive flight path.")]
		public readonly bool TerminalDive = false;

		[Desc("Turn speed while acquiring the terminal dive. Zero uses TurnSpeed.")]
		public readonly WAngle TerminalTurnSpeed = WAngle.Zero;

		[Desc("Maximum speed during a terminal dive. Zero keeps the legacy unlimited behavior when TerminalDive is disabled.")]
		public readonly WDist MaxHitSpeed = WDist.Zero;
		...
		void IRulesetLoaded<ActorInfo>.RulesetLoaded(Ruleset rules, ActorInfo info)
		{
			...
			if (PrepareTick <= 0)
				throw new YamlException($"PrepareTick must be greater than zero for terminal dive actor {info.Name}.");

			var terminalTurnSpeed = TerminalTurnSpeed != WAngle.Zero ? TerminalTurnSpeed : TurnSpeed;
			if (terminalTurnSpeed == WAngle.Zero)
				throw new YamlException($"Effective terminal turn speed must be greater than zero for actor {info.Name}.");

			if (MaxHitSpeed.Length <= 0)
				throw new YamlException($"MaxHitSpeed must be greater than zero for terminal dive actor {info.Name}.");
		}
