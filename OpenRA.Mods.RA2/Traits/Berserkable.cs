...
{
	[Desc("When enabled, the actor will try to attack nearby allied actors.")]
	...
	public class Berserkable : ConditionalTrait<BerserkableInfo>, INotifyIdle, INotifyCreated
	{
		...
		void INotifyIdle.TickIdle(Actor self)
		{
			...
			var targets = self.World.FindActorsInCircle(self.CenterPosition, range)
				.Where(a => a != self && a.IsTargetableBy(self) && !Info.TargetTypesToIgnore.Overlaps(a.GetEnabledTargetTypes()))
				.ToArray();
