...
namespace OpenRA.Mods.RA2.Traits
{
	...
	public sealed class ChronoResourceDelivery : INotifyHarvestAction, INotifyDockClientMoving, ITick
	{
		...
		void INotifyDockClientMoving.MovingToDock(Actor self, Actor hostActor, IDockHost host)
		{
			...
			if (!destination.HasValue || destination.Value != targetCell)
				ticksTillCheck = 0;
		...
		void TeleportIfPossible(Actor self)
		{
			...
			{
				var targetRefinery = refinery;
				var targetDock = dockHost;
				self.CancelActivity();
				self.QueueActivity(new ChronoResourceTeleport(dest, info));
				self.QueueActivity(new MoveToDock(self, targetRefinery, targetDock, forceEnter: true));
				Reset();
			}
		}
