...
namespace OpenRA.Mods.RA2.Traits
{
	...
	public class ExplodesForMaster : ConditionalTrait<ExplodesForMasterInfo>, INotifyKilled, INotifyDamage
	{
		...
		{
			using var deathTrace = DiagnosticTrace.Scope(DiagnosticSubsystem.Simulation, "ExplodesForMaster");
			deathTrace.Progress(self.ActorID, 0);
			...
			if (weapon == null)
				return;
			var weaponName = ReferenceEquals(weapon, Info.WeaponInfo) ? Info.Weapon : Info.EmptyWeapon;
			using var weaponTrace = DiagnosticTrace.Scope(DiagnosticSubsystem.Simulation,
				"Death.ExplosionWeapon.Master." + weaponName);
			weaponTrace.Progress(self.ActorID, 0);
			...
			// Port note: AS used MindControllable.MasterWhenDie (retained master after unlink);
			// our MindControllable only exposes the live Master link, which is close enough here.
			foreach (var mindControllable in self.TraitsImplementing<MindControllable>())
				if (mindControllable.Master != null && !mindControllable.Master.IsDead)
				{
					attacker = mindControllable.Master;
			...
			var args = new ProjectileArgs
			{
				...
				// Port note: release-20250330 IFirepowerModifier has no armament-name overload;
				// modifiers apply to all armaments, which is fine for single-armament launchers.
