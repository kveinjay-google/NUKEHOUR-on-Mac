mod-windowtitle = NUKE HOUR

modcontent-installprompt =
    Red Alert 2 requires artwork and audio files from the original game.

    Advanced Install includes options for copying the music, videos, and other content from an original game disc or digital installation.
modcontent-header =
    Game content may be extracted from the original game discs or an existing digital install.
modcontent-package-basefiles = Base Game Files
modcontent-package-music = Game Music
modcontent-package-yurifiles = Yuri's Revenge Expansion Files
modcontent-package-yurimusic = Yuri's Revenge Music
