...
python3 "${TEMPLATE_ROOT}/packaging/nukehour_version.py" check
CANONICAL_VERSION=$(python3 -c "import json; print(json.load(open('${TEMPLATE_ROOT}/packaging/nukehour-version.json'))['version'])")
CANONICAL_BUILD=$(python3 -c "import json; print(json.load(open('${TEMPLATE_ROOT}/packaging/nukehour-version.json'))['build'])")
...
require_variables "MOD_ID" "ENGINE_DIRECTORY" "PACKAGING_MACOS_DISPLAY_NAME" "PACKAGING_MACOS_INSTALLER_NAME" "PACKAGING_COPY_CNC_DLL" "PACKAGING_COPY_D2K_DLL" \
...
if [ "${TAG}" != "${CANONICAL_VERSION}" ]; then
	echo >&2 "Release tag ${TAG} does not match canonical macOS version ${CANONICAL_VERSION}."
	exit 1
fi
if [ $# -eq "1" ]; then
...
# An interrupted earlier build may contain resources that have since been
# removed from the source tree.  Always start from an empty, validated staging
# directory so deleted brand assets cannot leak into a new installer.
if [ "${BUILTDIR}" != "${PACKAGING_DIR}/build" ]; then
	echo >&2 "Refusing to clean unexpected macOS staging directory: ${BUILTDIR}"
	exit 1
fi
rm -rf "${BUILTDIR}"
...
/usr/libexec/PlistBuddy -c "Set :CFBundleVersion ${CANONICAL_BUILD}" "${LAUNCHER_CONTENTS_DIR}/Info.plist"
...
modify_plist "{MOD_NAME}" "${PACKAGING_MACOS_DISPLAY_NAME}" "${LAUNCHER_CONTENTS_DIR}/Info.plist"
...
if [ -e "${LAUNCHER_RESOURCES_DIR}/mods/${MOD_ID}/uibits/loadscreen.png" ]; then
	echo >&2 "Legacy loading artwork leaked into the NUKE HOUR macOS bundle."
	exit 1
fi

if ! grep -Fq $'\tImage: ra2|uibits/NUCLEAR-CRISIS-BG-06.png' "${LAUNCHER_RESOURCES_DIR}/mods/${MOD_ID}/mod.yaml"; then
	echo >&2 "The NUKE HOUR macOS bundle does not reference the canonical loading artwork."
	exit 1
fi

if ! cmp -s "${TEMPLATE_ROOT}/mods/${MOD_ID}/uibits/NUCLEAR-CRISIS-BG-06.png" \
	"${LAUNCHER_RESOURCES_DIR}/mods/${MOD_ID}/uibits/NUCLEAR-CRISIS-BG-06.png"; then
	echo >&2 "The NUKE HOUR macOS bundle contains a stale canonical loading artwork file."
	exit 1
fi

set_engine_version "${ENGINE_VERSION}" "${LAUNCHER_RESOURCES_DIR}"
MOD_VERSION=$(grep 'Version:' "${LAUNCHER_RESOURCES_DIR}/mods/${MOD_ID}/mod.yaml" | awk '{print $2; exit}')
if [ "${MOD_VERSION}" != "nukehour-storage-v1" ]; then
	echo >&2 "Unexpected NUKE HOUR storage identity: ${MOD_VERSION}"
	exit 1
fi

# Reuse the verified NUKE HOUR icon installed by the desktop build.
cp "${TEMPLATE_ROOT}/launcher_assets/icon.icns" "${LAUNCHER_RESOURCES_DIR}/${MOD_ID}.icns"
...
hdiutil create "build.dmg" -format UDRW -volname "${PACKAGING_MACOS_DISPLAY_NAME}" -fs HFS+ -srcfolder "${BUILTDIR}"
...
mkdir "/Volumes/${PACKAGING_MACOS_DISPLAY_NAME}/.background/"
tiffutil -cathidpicheck "${ARTWORK_DIR}/macos-background.png" "${ARTWORK_DIR}/macos-background-2x.png" -out "/Volumes/${PACKAGING_MACOS_DISPLAY_NAME}/.background/background.tiff"

cp "${LAUNCHER_DIR}/Contents/Resources/${MOD_ID}.icns" "/Volumes/${PACKAGING_MACOS_DISPLAY_NAME}/.VolumeIcon.icns"

echo '
   tell application "Finder"
     tell disk "'${PACKAGING_MACOS_DISPLAY_NAME}'"
           open
...
cp "${LAUNCHER_DIR}/Contents/Resources/${MOD_ID}.icns" "/Volumes/${PACKAGING_MACOS_DISPLAY_NAME}/.VolumeIcon.icns"
SetFile -c icnC "/Volumes/${PACKAGING_MACOS_DISPLAY_NAME}/.VolumeIcon.icns"
SetFile -a C "/Volumes/${PACKAGING_MACOS_DISPLAY_NAME}"

chmod -Rf go-w "/Volumes/${PACKAGING_MACOS_DISPLAY_NAME}"
sync
sync
...
if [ -n "${MACOS_DEVELOPER_USERNAME}" ] && [ -n "${MACOS_DEVELOPER_PASSWORD}" ] && [ -n "${MACOS_DEVELOPER_IDENTITY}" ]; then
	...
	xcrun stapler staple "/Volumes/${PACKAGING_MACOS_DISPLAY_NAME}/${PACKAGING_OSX_APP_NAME}"
...
hdiutil convert "build.dmg" -format ULFO -ov -o "${OUTPUTDIR}/${PACKAGING_MACOS_INSTALLER_NAME}-${TAG}.dmg"
rm "build.dmg"
